...
const DEFAULT_SITE_ROWS: SiteContentRow[] = [
  ...
  {
    key: "footer_text",
    value: "© 2026 PROKIM Luxe Store · Crafted with passion",
    type: "textarea",
  ...
  function startDrag(e: React.PointerEvent, id: string, mode: "move" | "resize" | "rotate") {
    ...
    function onMove(ev: PointerEvent) {
      ...
      if (mode === "move") {
        patchLocal(
          id,
          { x: Math.max(0, startVals.x + dx), y: Math.max(0, startVals.y + dy) },
          false,
        );
      } else if (mode === "resize") {
        patchLocal(
    ...
    <>
      ...
      <div className="fixed inset-0 z-[9998] pointer-events-none">
        {contentTargets.map((target) => (
          <button
            key={target.key}
            onClick={() => {
              setSelectedSiteKey(target.key);
              setTab("content");
              setShowInspector(true);
            }}
            className={`absolute pointer-events-auto outline outline-1 outline-dashed transition ${
              selectedSiteKey === target.key
                ? "outline-gold bg-gold/10"
                : "outline-primary/70 hover:bg-primary/10"
            }`}
            ...
            <div
              ...
              className={`absolute cursor-move select-none pointer-events-auto ${
                sel
                  ...
                  : "outline outline-1 outline-dashed outline-gold/60"
              }`}
              style={{
                left: o.x,
                top: o.y,
                ...
                background: o.kind === "image" ? "transparent" : o.bg || "rgba(255,255,255,0.06)",
      ...
      <div className="fixed top-3 left-1/2 -translate-x-1/2 z-[9999] flex max-w-[calc(100vw-1rem)] items-center gap-1 overflow-x-auto px-2 py-1.5 rounded-xl bg-onyx/95 backdrop-blur border border-gold/40 shadow-luxe">
        ...
        <Button size="sm" variant="luxe" onClick={saveAll} title="บันทึก">
          ...
          {dirty.size + deletedIds.size > 0 && (
            <span className="text-xs">({dirty.size + deletedIds.size})</span>
          )}
        </Button>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <p className="text-[10px] text-muted-foreground">
                  แก้ของเดิมในหน้านี้ — ข้อความ/รูป/ปุ่มเปิด-ปิด
                </p>
                <Button size="sm" variant="luxe" onClick={saveSite} disabled={siteDirty.size === 0}>
                  <Save className="w-3 h-3" /> บันทึก
                  {siteDirty.size > 0 ? ` (${siteDirty.size})` : ""}
                </Button>
              </div>
              {siteRows.length === 0 && (
                <div className="text-xs text-muted-foreground py-4 text-center">
                  ยังไม่มีช่องที่แก้ได้ — เปิดในแท็บ "วางใหม่" เพื่อเพิ่มของลอย
                </div>
              )}
              {siteRows.map((r) => (
                <div
                  key={r.key}
                  className={`p-2 rounded border ${siteDirty.has(r.key) ? "border-gold/60 bg-gold/5" : "border-border bg-onyx/40"}`}
                >
                  <button
                    type="button"
                  ...
                  >
                    {r.label || r.key}{" "}
                    <span className="text-[9px] text-muted-foreground font-mono">· {r.type}</span>
                  </button>
                  {r.type === "boolean" ? (
                    <select
                      className="w-full bg-input border border-border rounded px-2 py-1 text-xs"
                      value={siteEdits[r.key] ?? "true"}
                      onChange={(e) => patchSite(r.key, e.target.value)}
                    >
            ...
            <div className="text-xs text-muted-foreground">
              ...
              <div className="mt-2 max-h-60 overflow-auto space-y-1">
                {items.map((o) => (
                  ...
                  >
                    {o.visible ? "👁" : "🚫"} [{o.kind}]{" "}
                    {o.label || o.content?.slice(0, 20) || "(ไม่มีชื่อ)"}
                  </button>
                ))}
                {items.length === 0 && (
                  <div className="py-4 text-center">ยังไม่มี — กด T / 🖼 / 🖱 ด้านบน</div>
                )}
              </div>
            </div>
          )}
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-1">
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => patchLocal(selected.id, { visible: !selected.visible })}
                  >
                    {selected.visible ? (
              ...
              <div className="grid grid-cols-2 gap-1.5 text-xs">
                <label>
                  ...
                  <Input
                    ...
                    type="number"
                    value={selected.x}
                    onChange={(e) => patchLocal(selected.id, { x: Number(e.target.value) })}
                  />
                </label>
                <label>
                  Y
                  <Input
                    ...
                    type="number"
                    value={selected.y}
                    onChange={(e) => patchLocal(selected.id, { y: Number(e.target.value) })}
                  />
                </label>
                <label>
                  กว้าง
                  <Input
                    ...
                    type="number"
                    value={selected.w}
                    onChange={(e) => patchLocal(selected.id, { w: Number(e.target.value) })}
                  />
                </label>
                <label>
                  สูง
                  <Input
                    ...
                    type="number"
                    value={selected.h}
                    onChange={(e) => patchLocal(selected.id, { h: Number(e.target.value) })}
                  />
                </label>
                <label>
                  หมุน°
                  <Input
                    ...
                    onChange={(e) => patchLocal(selected.id, { rotate: Number(e.target.value) })}
                  />
                </label>
                <label>
                  ตัวอักษร
                  <Input
                    ...
                    onChange={(e) => patchLocal(selected.id, { font_size: Number(e.target.value) })}
                  />
                </label>
                <label>
                  ชั้น z
                  <Input
                    ...
                    onChange={(e) => patchLocal(selected.id, { z_index: Number(e.target.value) })}
                  />
                </label>
                <label>
                  สีอักษร
                ...
                <label className="col-span-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => patchLocal(selected.id, { bg: e.target.value })}
                  />
                </label>
              </div>
              <Button
